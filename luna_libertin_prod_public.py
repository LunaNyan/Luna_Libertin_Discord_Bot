...
async def on_message(message):
    ...
    if message.content.startswith(test_glyph + '루냥아 도와줘'):
        ...
        a = a.replace('루냥아 도와줘', '')
        if a == '':
            embed=discord.Embed(title="기계식 루냥이를 초대해주셔서 감사합니다!", description="[민원창구](https://discordapp.com/invite/yyS9x5V) [봇 초대하기](https://discordapp.com/oauth2/authorize?client_id=598080777565241354&scope=bot&permissions=76864)", color=0xff0080)
            embed.set_author(name="기계식 루냥이 사용 방법",icon_url=client.user.avatar_url)
            embed.add_field(name="도움말", value="루냥아 도와줘 (항목), 루냥아 섯다 도와줘, 루냥아 업데이트내역", inline=False)
            embed.add_field(name="일상", value="루냥아 배고파, 루냥이 귀여워, 루냥이 쓰담쓰담, 와! 샌즈!", inline=False)
