import sys, discord, cpuinfo, psutil, os, math, datetime, m_food, m_lang
...
copyright_text = "ⓒ 2017 libertin, 2019 - " + str(d.year) + " STUDIO ONE"

def refresh_year():
    global d, copyright_text
    d = datetime.date.today()
    copyright_text = "ⓒ 2017 libertin, 2019 - " + str(d.year) + " STUDIO ONE"
...
def help(user, client, text, bot_ver, head, mode, generic_help=False):
    ...
    if text == head + '도와줘' or generic_help == True:
        ...
        embed.add_field(name="커뮤니티", value="루냥아 공지사항 목록, 루냥아 공지사항 (숫자), 루냥아 방명록, 루냥아 방명록 쓰기 (할 말), 루냥아 서버목록, 루냥아 서버랭킹, 루냥아 호감도랭킹, 루냥아 포스트 (번호), 루냥아 포스트 작성 (임베드 코드), 루냥아 임베드 목록", inline=False)
        embed.add_field(name="프로필", value="루냥아 출석체크, 루냥아 출석체크 초기화, 루냥아 나 어때, 루냥아 (멘션) 어때, 루냥아 소개말 (자기소개), 루냥아 거울 (멘션), 루냥아 닉변 (닉네임), 루냥아 생성일시공개, 루냥아 디데이, 루냥아 디데이 추가, 루냥아 디데이 삭제, 루냥아 반모, 루냥아 유저 접두어 (접두어)", inline=False)
        ...
        embed.add_field(name="루냥아 포스트 (번호)", value="목록 내 번호에 해당하는 포스트를 불러옵니다", inline=False)
        embed.add_field(name="루냥아 포스트 작성 (임베드 코드)", value="임베드를 포스트 목록에 게시합니다", inline=False)
        embed.add_field(name="루냥아 포스트 목록", value="게시된 포스트의 목록을 불러옵니다", inline=False)
    elif a == '프로필':
        ...
        embed.add_field(name="루냥아 출석체크 초기화", value="출석 체크 횟수를 초기화합니다 **(사전 경고 없이 이루어지니 신중하게 실행하세요!)**", inline=False)
        ...
        embed.add_field(name="루냥아 디데이", value="저장된 디데이 목록을 표시합니다", inline=False)
        embed.add_field(name="루냥아 디데이 추가", value="디데이를 추가합니다", inline=False)
        embed.add_field(name="루냥아 디데이 삭제", value="저장된 디데이를 목록에서 삭제합니다", inline=False)
        ...
        embed.add_field(name="루냥아 임베드", value="커스텀 임베드 관련 도움말을 볼 수 있습니다", inline=False)
        ...
        embed.add_field(name="수정노기 설정", value="수정노기 설정을 진행합니다", inline=False)
        embed.add_field(name="수정노기 (횟수)", value="수정노기 시뮬레이션을 진행합니다", inline=False)
        ...
        embed.add_field(name="루냥아 관심 가져주기 커스텀", value='관심 가져주기를 통해 나오는 텍스트를 커스텀할 수 있습니다', inline=False)
        ...
        if a == '관리자':
            ...
            embed.add_field(name="채널관리", value="루냥아 환영인사 (메시지), 루냥아 작별인사 (메시지), 루냥아 금지채널 추가, 루냥아 금지채널 삭제, 루냥아 야짤채널, 루냥아 비밀채널 (숫자), 루냥아 잠수채널", inline=False)
            ...
            embed.add_field(name="명령어", value="루냥아 서버 접두어 (접두어), 루냥아 일상대화 접두어, 루냥아 서버 지정 명령어 접두어, 루냥아 커스텀 명령어 연결, 루냥아 커스텀 명령어 연결목록, 루냥아 커스텀 명령어 연결해제", inline=False)
            embed.add_field(name="설정", value="루냥아 서버공개, 루냥아 가입일시 전체공개, 루냥아 불타는 서버 토글, 루냥아 불타는 서버 문구, 루냥아 잠수 쿨타임, 루냥아 서버소개, 루냥아 서버코드 초기화, 루냥아 잠수금지, 루냥아 확성기금지", inline=False)
            embed.add_field(name="기타", value="루냥아 초대링크 생성, 루냥아 자가진단", inline=False)
            # 기타 탭에 서버이동, 서버초기화는 원래 안뜨도록 되어 있음!
        elif a == '관리자 전용채널':
            ...
            embed.add_field(name="루냥아 잠수채널", value="현재 채널을 잠수 알림 채널로 설정합니다", inline=False)
        elif a == '관리자 채널연결':
            ...
            embed=discord.Embed(title="관리자 기능 도움말", description="명령어", color=0xff0000)
            ...
            embed.add_field(name="루냥아 커스텀 명령어 연결", value="다른 서버의 커스텀 명령어를 가져와 동기화합니다", inline=False)
            embed.add_field(name="루냥아 커스텀 명령어 연결목록", value="커스텀 명령어 연결 목록을 보여줍니다", inline=False)
            embed.add_field(name="루냥아 커스텀 명령어 연결해제", value="커스텀 명령어 연결을 해제합니다", inline=False)
        elif a == '관리자 설정':
            ...
            embed.add_field(name="루냥아 서버코드 초기화", value="서버 코드를 재생성합니다", inline=False)
            embed.add_field(name="루냥아 잠수금지", value="잠수 기능 사용 가능 여부를 결정합니다", inline=False)
            embed.add_field(name="루냥아 확성기금지", value="확성기 기능 사용 가능 여부를 결정합니다", inline=False)
        elif a == '관리자 기타':
            ...
            embed.add_field(name="루냥아 서버이동 (서버코드)", value="봇에 내장된 커스텀 명령어와 설정을 다른 서버로 이전합니다", inline=False)
            embed.add_field(name="루냥아 서버초기화", value="봇에 있는 서버 데이터를 초기화합니다", inline=False)
        else:
...
def get_info_public(uptime, servername, bot_ver):
    ...
    embed.add_field(name="CPU", value=cpuinfo.get_cpu_info()["brand"], inline=True)
    embed.add_field(name="RAM 용량", value=str(int(psutil.virtual_memory().total / 1048576)))
    embed.add_field(name="봇 시스템 버전", value=bot_ver, inline=False)
    embed.add_field(name="Python 버전", value=cpuinfo.get_cpu_info()["python_version"], inline=True)
...
def bday():
    embed=discord.Embed(title="저는 2017년 5월 9일에 태어났어요!", description="그리고 저의 모습은 2014년에 구상되었어요!", color=0xffff00)
...
def selfintro(client, bot_ver, message):
    ...
    text+= "[Kwabang](https://twitter.com/kwabang2827)\n"
    text+= "[Lemongreen](https://twitter.com/starterdroid)\n"
    embed.add_field(name="개발에 도움을 주신 분들", value=text, inline=True)
    text = "작관존잘\n"
    text+= "[perillamint](https://twitter.com/perillamint)"
    embed.add_field(name="인프라 및 시스템 관리자", value=text, inline=True)
    text = "[Scatter Lab](https://scatterlab.co.kr)\n"
    text+= "[DJGONI](https://twitter.com/IRP75177144)\n"
    text+= "[FOX-B](https://twitter.com/FOXB_)\n"
    text+= message.author.name + "#" + message.author.discriminator
    embed.add_field(name="기능 제공 및 특별 감사", value=text, inline=True)
    embed.add_field(name="프로필 이미지", value="해당 봇의 프로필 이미지는 [ちみうちのこめーかー](https://picrew.me/image_maker/364109)로 제작되었습니다\n봇의 제작자는 妙子로부터 아이콘 이미지로서의 일러스트 사용을 허가받았습니다", inline=False)
    embed.set_thumbnail(url=client.user.avatar_url)
    embed.set_footer(text=copyright_text + " | ver " + bot_ver)
    return embed
...
def rules():
    ...
    text+= "단, 상대방 간 상호작용 기능 사용으로 인해 발생하는 봇 외적 문제에 대해서는 제작자가 책임지지 않으며, (버그 등 봇 내적 문제는 제외)\n"
    text+= "또한, 이를 이용하여 __다음과 같은 행동을 하는 것은 기본적으로 금지__되며, **적발 시 이용 제한 조치, 사례가 심한 경우 법적 조치**에 처하실 수 있음을 알려드립니다.\n\n"
    text+= "- **미성년자의 야짤 기능 사용**\n"
    ...
    text+= "사용에 주의가 필요한 컨텐츠 : 섯다, 가위바위보, 로또, 로하이, 슬롯머신 게임(사행성), 짤, 야짤(선정성)"
